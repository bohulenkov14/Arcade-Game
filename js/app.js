...
Player.prototype.handleInput = function(keyCode) {
  ...
  currentLevel.bonuses.forEach(function(bonus) {
    if (this.checkBonusCollision(bonus)) {
        currentScore += 100;
...
var GuiObject = function(text, x, y) {
  this.x = x;
  this.y = y;
  this.text = text;
...
GuiObject.prototype.update = function(dt) {
};

GuiObject.prototype.render = function() {
...
var guiLevel = new Level(
  ...
  [],
  [],
  [new GuiObject("New Game", 350, 350),
   new GuiObject("Choose Map", 350, 450)]
  );

var gameState = "MainMenu"

var currentLevel = guiLevel;
...
document.addEventListener('keyup', function(e) {
    ...
    gameLevel.players.forEach(function(player) {
...
Resources.onReady(function() {
    currentLevel.enemies.forEach(placeEnemyToStartPosition);
    currentLevel.players.forEach(placePlayerToStartPosition);
    currentLevel.bonuses.forEach(placeBonusOnMap);
});
